# SPI Server Template Application

This application serves as an **SPI Server** running on any development board with CMSIS compliant **SPI driver** and compatible **Board Layer**.  
...
Insure that proper **Driver_SPI#** is selected in the **`SPI_Server_Config.h`** file with define `SPI_SERVER_DRV_NUM`.

> **Note:** By default, if defined by the board layer, the `ARDUINO_UNO_SPI` will be used as selected **Driver_SPI#**.

Respective pins depend on the selected board, check the board description for pins used for the SPI interface.

> **Note:** Also ground pin must be connected to the DUT.

> **Note:** If the SPI driver is not fully CMSIS compliant then SPI Server most likely will not work properly and reliably.
