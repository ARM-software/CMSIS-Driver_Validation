# USART Server Template Application

This application serves as a **USART Server** running on any development board with CMSIS compliant **USART driver** and compatible **Board Layer**.  
...
For more information about **Board Layer** check the [**Reference Applications documentation**](https://open-cmsis-pack.github.io/cmsis-toolbox/ReferenceApplications/).
...
Insure that proper **Driver_USART#** is selected in the **`USART_Server_Config.h`** file with define `USART_SERVER_DRV_NUM`.

> **Note:** By default, if defined by the board layer, the `ARDUINO_UNO_UART` will be used as selected **Driver_USART#**.

If USART Server requires DCD and RI pin functionality testing then adapt **`USART_Server_HW.c`** file accordingly.

Respective pins depend on the selected board, check the board description for pins used for the USART interface.

> **Note:** Also ground pin must be connected to the DUT.

> **Note:** If the USART driver is not fully CMSIS compliant then USART Server most likely will not work properly and reliably.
...
| Target      | Description
|-------------|------------
| **Debug**   | Low optimization build with STDOUT interface (for debugging).
